...
public class BookDao {
    ...
    private static final String FIND_BY_ID = FIND_ALL + " WHERE b.id = ?";

    private static final String FIND_BY_GENRE = FIND_ALL + " WHERE g.id= ?";

    private static final String FIND_BY_NAME = FIND_ALL + " WHERE b.name= ?";

    private static final String FIND_BY_USER = FIND_ALL + " WHERE u.name= ?";

    private static final String FIND_BY_USER_ID = FIND_ALL + " WHERE user_id= ?";

    private static final String FIND_BY_USER_GENRE = FIND_ALL + " WHERE u.name= ? AND g.id=?";

    private static final String FIND_BY_BOOK_GENRE = FIND_ALL + " WHERE b.name= ? AND g.id=?";
    ...
    public List<Book> findAllPage(int numberPage) {
        List<Book> books = new ArrayList<>();
        try (Connection connection = ConnectionPool.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(FIND_BY_BOOK_PAGE)) {
            preparedStatement.setInt(1, numberPage*10);
    ...
    public List<Book> findByGenreUser(int genreId, String nameUser) {
        List<Book> books = new ArrayList<>();
        try (Connection connection = ConnectionPool.getConnection();
