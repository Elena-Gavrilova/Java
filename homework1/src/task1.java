...
//        System.out.println(isDateTest(day, month, year));
        ...
        {
            isNextDay(day, month, year);//isNextDay-находит следующий день
        } else {
    ...
    private static void isNextDay(int day, int month, int year) {
...
//        System.out.println(isDateTest(day, month, year));
    ...
    private static boolean isDateTest(int day, int month, int year) {
        boolean result=year >= 1 ? isTestDay(day, month, year) == true && isTestMonth(month) == true ? true : false : false;
        return result;
    }
    ...
    private static boolean isDayMonth31(int month) {
        boolean result = (month <= 7 && month % 2 == 1) || (month >= 8 && month % 2 == 0);
        return result;
    }
    ...
    private static boolean isTestDay(int day, int month, int year) {
        boolean result= ((month == 2) ?
                        (isTestLeapYear(year)?
                                ((day >= 1 && day <= 29) ? true : false) : ((day >= 1 && day <= 28) ? true : false)) :
                        (isDayMonth31(month) ?
                                ((day >= 1 && day <= 31) ? true : false) : ((day >= 1 && day <= 30) ? true : false)));
        return result;
    }
